    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.datagurus.update", [$dataguru->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="nama">{{ trans('cruds.dataguru.fields.nama') }}</label>
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" nama="nama" id="nama" value="{{ old('nama', $dataguru->nama) }}" required>
                @if($errors->has('nama'))
                    <div class="invalid-feedback">
                        {{ $errors->first('nama') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.dataguru.fields.nama_helper') }}</span>
            </div>
            <div class="form-group">
